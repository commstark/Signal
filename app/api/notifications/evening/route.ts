...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    console.error('evening cron: push_subscriptions read failed', error);
  ...
  const perUser = await Promise.all(
    userIds.map(async (uid) => {
      try {
        const result = await sendEveningPrompt(uid);
  ...
  console.log(
    `evening cron complete: users=${userIds.length} attempted=${totals.attempted} succeeded=${totals.succeeded} removed_dead=${totals.removed_dead} failed_users=${totals.failed_users} duration_ms=${Date.now() - started}`,
  );
